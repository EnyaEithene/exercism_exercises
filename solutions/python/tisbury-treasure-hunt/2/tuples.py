...
def clean_up(combined_record_group):
    ...
    for record in combined_record_group:
        report += "("
        for index_item,item in enumerate(record):
            if index_item == 1:                    # Skip unnecessary coordinates
                continue
                
            if isinstance(item, str):              # Add quotes for strings in formatting
                report += f"'{item}'"
            else:
                report += f"{item}"

            if index_item == len(record) - 1:      # Add middle separator or row separator
                report += ")\n"
            else:
                report += ", "
